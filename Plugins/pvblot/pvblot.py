...
class _PVBlotInterp(cmd.Cmd):
    """pvblot - A blot-like interpreter using Visocyte as a back end.

    The pvblot interpreter reads commands in a manner similar to the SEACAS
    blot command and performs visualization using the Visocyte framework.
...
def initialize(filename=None):
    ...
    wasProgressPrinting = visocyte.servermanager.GetProgressPrintingIsEnabled()
    visocyte.servermanager.SetProgressPrintingEnabled(False)
...
def finalize():
    ...
    import visocyte.servermanager
    global wasProgressPrinting
    visocyte.servermanager.SetProgressPrintingEnabled(wasProgressPrinting)
