...
class _State(object):
    ...
    def __del__(self):
        ...
        # Sometimes visocyte is already unloaded at this point.
        if not visocyte: return

        # Remove pipeline objects
        for name in self._pipelineObjects.keys():
            source = self._pipelineObjects.pop(name)
            visocyte.simple.Delete(source)

        # Finally remove the reader
        visocyte.simple.Delete(self.reader)
    ...
    def _getPipelineObject(self, name, createFunc, **newObjectOptions):
        if not self._pipelineObjects.has_key(name):
            ...
            visocyte.simple.Hide(self._pipelineObjects[name])
    ...
    def _getReader(self):
        ...
        if not self._reader:
            ...
            if visocyte.servermanager.vtkSMReaderFactory.CanReadFile(\
              self.filename, "sources", "spcthreader",\
              visocyte.servermanager.ActiveConnection.ID):
                # For the spy plot reader we will turn on all variables
                # and then apply the Extract CTH Parts filter to give us
                # something to work with
                r = visocyte.simple.SpyPlotReader(FileName=self.filename)
                r.CellArrays = r.CellArrays.Available
                extract = visocyte.simple.ExtractCTHParts()
                material_vars = []
                ...
                self._reader = visocyte.simple.ExodusIIReader(FileName=self.filename)
    ...
    def resetPipeline(self):
        for obj in self._pipelineObjects.values():
            if obj:
                visocyte.simple.Hide(obj)
                visocyte.simple.SetDisplayProperties(obj, Opacity=1)
    ...
    # visocyte.simple is improved.
    _blockLookupTable = None
    def _getBlockLookupTable(self):
        if not self._blockLookupTable:
            self._blockLookupTable = visocyte.servermanager.rendering.PVLookupTable()
            visocyte.servermanager.Register(self._blockLookupTable)
    ...
    def _getSpectrumLookupTable(self):
        if not self._spectrumLookupTable:
            self._spectrumLookupTable = visocyte.servermanager.rendering.PVLookupTable()
            visocyte.servermanager.Register(self._spectrumLookupTable)
    ...
    def _rebuildSpectrumLookupTable(self, lt):
        ...
        for i in xrange(self.numSpectrumColors+1):
            ...
            color = visocyte.vtk.vtkMath.HSVToRGB(2.0/3.0 - i*hueStep, 1, 1)
...
def _init_blotish(filename):
    ...
    state.renderview = visocyte.simple.GetRenderView()
...
def detour():
    ...
    visocyte.simple.SetActiveView(state.renderview)
    wireframe()
...
def tplot():
    ...
    visocyte.simple.SetActiveView(state.tplot.view)
...
def _updateMeshRender():
    ...
    todraw = state.reader
    if state.mlines:
        visocyte.simple.Show(todraw)
        if state.msurface:
            visocyte.simple.SetDisplayProperties(
                                     todraw, Representation="Surface With Edges")
        else:
            visocyte.simple.SetDisplayProperties(todraw,
...
def contour(variable=None):
    ...
    contour.Isosurfaces = state.spectrumScalarValues
    visocyte.simple.Show(contour)
    visocyte.simple.SetDisplayProperties(contour,
                                         Representation="Surface",
                                         ColorAttributeType=state.currentVariableType,
                                         ColorArrayName=state.currentVariable,
                                         LookupTable=state.spectrumLookupTable)
...
def vector(variable=None):
    ...
    state.resetPipeline()
    # Show the glyphs.
    visocyte.simple.Show(glyph)
    # Show a transparent surface.
    state.mlines = 0
    state.msurface = 1
    surface = _updateMeshRender()
    visocyte.simple.SetDisplayProperties(surface,
                                         ColorAttributeType=state._block_id_field,
                                         ColorArrayName=state._block_id_variable,
                                         LookupTable=state.blockLookupTable,
                                         Opacity=0.5)
...
def _tplot_plot(io_helper=None):
    if state.tplot.overlay:
        ...
        if state.diskwrite:
            ...
            visocyte.simple.WriteImage(fname, state.tplot.view)
...
def _detour_plot(io_helper=None):
    ...
    for stepIdx in xrange(nSteps):
        t = plotTimes[stepIdx]
        view.ViewTime = t
        visocyte.simple.Render(view)

        if state.diskwrite:
            visocyte.simple.WriteImage(state.get_next_screenshot_filename("detour"), view)
...
def rotate(*rotations):
    ...
    unique prefix.
    """
    camera = visocyte.simple.GetActiveCamera()
...
def translate(x, y):
    ...
    x = float(x)
    y = float(y)
    from visocyte.vtk import vtkMath
    camera = visocyte.simple.GetActiveCamera()
