"""
Module that looks at a Visocyte pipeline and automatically creates a cinema
...
from ...adaptors.visocyte import pv_explorers
from ...adaptors.visocyte import progress
...
def record_visibility():
    """
    at start of run, record the current visocyte state so we can return to it
    """
    proxies = []

    view_info = {}
    view_proxy = visocyte.simple.GetActiveView()
    ...
    source_proxies = visocyte.servermanager.ProxyManager().GetProxiesInGroup(
        "sources")
    ...
    for key in source_proxies:
        ...
        visocyte.simple.Hide3DWidgets(proxy=proxy)
        listElt['proxy'] = proxy
        ...
        rep = visocyte.servermanager.GetRepresentation(proxy, view_proxy)
        if rep is not None:
...
def restore_visibility(proxies):
    """at end of run, return to a previously recorded visocyte state"""
    view_proxy = visocyte.simple.GetActiveView()
    ...
    for listElt in proxies:
        if listElt['proxy'] == "__view_info":
            ...
            if vis is not None:
                rep = visocyte.servermanager.GetRepresentation(
                    proxy, view_proxy)
                if rep is not None:
                    ...
                    if listElt['color_array_association']:
                        rep.SetScalarColoring(
                            ...
                            visocyte.servermanager.GetAssociationFromString(
                                listElt['color_array_association']))
...
def get_pipeline():
    ...
    for proxy in proxies:
        source = visocyte.simple.FindSource(proxy['name'])
...
def add_filter_value(name, cs, userDefinedValues):
    ...
    source = visocyte.simple.FindSource(name)

    # Determine the filter type
    valueType = None
    if isinstance(source, visocyte.simple.servermanager.filters.Clip):
        valueType = 'OffsetValues'
        explorerDir[name] = pv_explorers.Clip(name, source)
    elif isinstance(source, visocyte.simple.servermanager.filters.Slice):
        valueType = 'SliceOffsetValues'
        explorerDir[name] = pv_explorers.Slice(name, source)
    elif isinstance(source, visocyte.simple.servermanager.filters.Contour):
...
def filter_has_parameters(name):
    ...
    return any(imap(lambda filter: isinstance(source, filter),
                    [visocyte.simple.servermanager.filters.Clip,
                     visocyte.simple.servermanager.filters.Slice,
                     visocyte.simple.servermanager.filters.Contour]))
...
def add_control_and_colors(name, cs, userDefined, arrayRanges, disableValues):
    ...
    defaultName = None
    view_proxy = visocyte.simple.GetActiveView()
    rep = visocyte.servermanager.GetRepresentation(source, view_proxy)
...
def update_all_ranges(cs, arrayRanges):
    ...
    This version happens in visocyte export for time varying data
    ...
    for pname, param in plist.items():
        if 'valueRanges' in param:
            ...
            proxy = visocyte.simple.FindSource(filtername)  # get the filter
...
def make_cinema_store(proxies,
    ...
    if forcetime:
        ...
        times = visocyte.simple.GetAnimationScene().TimeKeeper.TimestepValues
        if times:
...
def explore(cs, proxies, iSave=True, currentTime=None, userDefined={},
    ...
    view_proxy = visocyte.simple.GetActiveView()
    dist = visocyte.simple.GetActiveCamera().GetDistance()
    ...
    for x in proxies:
        ...
        for y in params:
            ...
            if name in y:
                ...
                sp = visocyte.simple.FindSource(name)
                if specLevel != "A":
                    rep = visocyte.servermanager.GetRepresentation(
                        sp, view_proxy)
    ...
    cam = visocyte.simple.GetActiveCamera()
    ...
    if 'object' in tracking:
        ...
        tracked_source = visocyte.simple.FindSource(objname)
        if tracked_source is None:
            name_upper = objname[0].upper() + objname[1:]
            tracked_source = visocyte.simple.FindSource(name_upper)
    ...
    if not times:
        ...
        for t in times:
            ...
            visocyte.simple.Render(view_proxy)
...
def export_scene(baseDirName, viewSelection, trackSelection, arraySelection):
    ...
    - viewSelection (following the format defined in
      Wrapping/Python/visocyte/cpstate.py):
    ...
    '''
    # save initial state
    initialView = visocyte.simple.GetActiveView()
    ...
    for viewName, viewParams in viewSelection.iteritems():
        ...
        view = visocyte.simple.FindView(viewName)
        visocyte.simple.SetActiveView(view)
    ...
    visocyte.simple.SetActiveView(initialView)
...
def prepare_selection(trackSelection, arraySelection):
    ...
    for name, values in trackSelection.iteritems():
        ...
        if (source is None):
            ...
            source = visocyte.simple.FindSource(name_upper)
