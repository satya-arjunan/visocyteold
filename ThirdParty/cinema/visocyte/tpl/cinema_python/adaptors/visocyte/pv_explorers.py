"""
Module consisting of explorers and tracks that connect arbitrary visocyte
...
import visocyte.simple as simple
from visocyte import numpy_support as numpy_support
...
class ImageExplorer(explorers.Explorer):
    """
    An Explorer that connects a Visocyte script's views to a store.
    Basically it iterates over the parameters and for each unique combination
    it tells Visocyte to make an image and saved the result into the store.
    """
...
class Camera(explorers.Track):
    """
    A track that connects a Visocyte script's camera to the phi and theta
...
class PoseCamera(explorers.Track):
    """
    A track that connects a Visocyte script's pose track which has 3x3
    ...
    def execute(self, document):
        ...
        # at every timestep visocyte records camera location
...
class ColorList():
    """
    A helper that creates a dictionary of color controls for Visocyte.
...
class Color(explorers.Track):
    ...
    def execute(self, doc):
        """tells Visocyte to color the object we've been assigned
