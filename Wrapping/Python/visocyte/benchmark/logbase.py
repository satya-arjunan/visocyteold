...
def maximize_logs () :
    """
    Convenience method to ask visocyte to produce logs with lots of space and
    highest resolution.
    """
    ...
    ss = visocyte.servermanager.vtkSMSession
    ...
    pxm = visocyte.servermanager.ProxyManager()
...
def import_logs( filename ) :
    ...
    TODO: add an option to load in raw visocyte logs in text format
    """
    import pickle
    global logs
    logs = []
...
def get_logs() :
    ...
    connectionId = visocyte.servermanager.ActiveConnection.ID
    session = visocyte.servermanager.ActiveConnection.Session
    ...
    for component in components:
        timerInfo = visocyte.servermanager.vtkPVTimerInformation()
...
def test_module():
    ...
    visocyte.servermanager.SetProgressPrintingEnabled(0)
