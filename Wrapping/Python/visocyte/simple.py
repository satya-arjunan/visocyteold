r"""simple is a module for using visocyte server manager in Python. It
...
import visocyte._backwardscompatibilityhelper

# Bring OutputPort in our namespace.
from visocyte.servermanager import OutputPort
...
def Interact(view=None):
    ...
    visocyte.print_debug_info("Staring interaction. Use 'q' to quit.")
...
def OpenDataFile(filename, **extraArgs):
    ...
    xml_name = visocyte.make_name_valid(prototype.GetXMLLabel())
...
def ImportCinema(filename, view=None):
    ...
    """
    try:
        from visocyte.modules.vtkPVCinemaReader import vtkSMCinemaDatabaseImporter
    except ImportError:
...
def AssignLookupTable(arrayObject, LUTName, rangeOveride=[]):
    ...
    may ber obtained from a Visocyte source in it's point or cell data.
    The lookup tables available in Visocyte's GUI are loaded by default.
...
def LoadXML(xmlstring, ns=None):
    ...
    If you loaded the simple module with ``from visocyte.simple import *``,
...
def LoadCustomFilters(filename, ns=None):
    ...
    ``from visocyte.simple import *``, make sure to pass ``globals()`` as an
    argument.
    """
...
def ResetProperty(propertyName, proxy=None, restoreFromSettings=True):
    ...
    if propertyToReset != None:
        ...
        if restoreFromSettings:
            settings = visocyte.servermanager.vtkSMSettings.GetInstance()
...
if not visocyte.options.satelite:
