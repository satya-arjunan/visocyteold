r"""
Internal module used by visocyte.servermanager to help warn about properties
changed or removed.
...
NotSupportedException = visocyte.NotSupportedException
...
def setattr(proxy, pname, value):
    ...
    1. If `visocyte.compatibility.GetVersion()` is less than the version in
    ...
    """
    version = visocyte.compatibility.GetVersion()

    if pname == "ColorAttributeType" and proxy.SMProxy.GetProperty("ColorArrayName"):
        if visocyte.compatibility.GetVersion() <= 4.1:
    ...
    if pname == "LockScalarRange" and proxy.SMProxy.GetProperty("AutomaticRescaleRangeMode"):
        if visocyte.compatibility.GetVersion() <= 5.4:
            if value:
                from visocyte.modules.vtkPVServerManagerRendering import vtkSMTransferFunctionManager
    ...
    if pname in ["FieldAssociation", "GenerateCellConnectivity"] and proxy.SMProxy.GetXMLName() == "SpreadSheetRepresentation":
        if visocyte.compatibility.GetVersion() <= 5.5:
            raise Continue()
        else:
            raise NotSupportedException(
    ...
    if pname == "Input" and proxy.SMProxy.GetXMLName() == "ResampleWithDataset":
        if visocyte.compatibility.GetVersion() < 5.7:
...
def getattr(proxy, pname):
    ...
    deprecated and the visocyte compatibility version was set to a version older
    than when the property was deprecated.

    Will raise ``NotSupportedException`` if the property was deprecated and
    visocyte compatibility version is newer than that deprecation version.
