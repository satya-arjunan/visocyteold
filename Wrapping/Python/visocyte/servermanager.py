r"""servermanager is a module for using visocyte server manager in Python.
...
Usually users should use the visocyte.simple module instead as it provide a
more user friendly API.

A simple example::

  from visocyte.servermanager import *
...
import visocyte, re, os, os.path, types, sys, atexit

# prefer `vtk` from `visocyte` since it doesn't import all
# vtk modules.
from visocyte import vtk
from visocyte import _backwardscompatibilityhelper as _bc

from visocyte.modules.vtkPVServerImplementationCore import *
from visocyte.modules.vtkPVClientServerCoreCore import *
from visocyte.modules.vtkPVServerManagerCore import *

try:
  from visocyte.modules.vtkPVServerManagerDefault import *
except:
  visocyte.print_error("Error: Cannot import vtkPVServerManagerDefault")
try:
  from visocyte.modules.vtkPVServerManagerRendering import *
except:
  visocyte.print_error("Error: Cannot import vtkPVServerManagerRendering")
try:
  from visocyte.modules.vtkPVServerManagerApplication import *
except:
  visocyte.print_error("Error: Cannot import vtkPVServerManagerApplication")
try:
...
def _wrap_property(proxy, smproperty):
    ...
    """
    property = None
    if visocyte.compatibility.GetVersion() >= 3.5 and \
...
class Proxy(object):
    ...
    def GetPropertyValue(self, name):
        ...
        if isinstance(p, VectorProperty):
            if visocyte.compatibility.GetVersion() <= 4.1 and name == "ColorArrayName":
    ...
    def __setattr__(self, name, value):
        try:
            ...
            visocyte.print_debug_info("No attribute %s" % name)
            ...
            visocyte.print_debug_info(name)
            try:
                setter(self, value)
            except ValueError:
                # Let the backwards compatibility helper try to handle this
...
class ExodusIIReaderProxy(SourceProxy):
    ...
    if visocyte.compatibility.GetVersion() >= 3.5:
        def FileNameChanged(self):
...
class ArrayInformation(object):
    ...
    if visocyte.compatibility.GetVersion() <= 3.4:
...
def LoadState(filename, connection=None):
    ...
    for view in views:
        ...
        # ViewSize property. In visocyte, the GUI takes care
        # of this.
...
def _make_name_valid(name):
    return visocyte.make_name_valid(name)
...
def _createClass(groupName, proxyName, apxm=None):
    ...
    if not proto:
       visocyte.print_error("Error while loading %s %s"%(groupName, proxyName))
       return None
    pname = proxyName
    if visocyte.compatibility.GetVersion() >= 3.5 and proto.GetXMLLabel():
...
# Needs to be called when visocyte module is loaded from python instead
...
if not vtkProcessModule.GetProcessModule():
    ...
    if visocyte.options.batch:
      ...
      # from master
      pm = vtkProcessModule.GetProcessModule()
      if not visocyte.options.symmetric and pm.GetPartitionId() != 0:
        visocyte.options.satelite = True
