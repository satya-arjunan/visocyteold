...
vtkSMSettings::vtkSMSettings()
{
  ...
  if (vtksys::SystemTools::GetEnv("PV_SETTINGS_DEBUG") != nullptr)
  {
    vtkWarningMacro("`PV_SETTINGS_DEBUG` environment variable has been deprecated."
                    "Please use `VISOCYTE_LOG_APPLICATION_VERBOSITY=INFO` instead.");
...
bool vtkSMSettings::AddCollectionFromString(const std::string& settings, double priority)
{
  ...
  vtkVLogF(
    VISOCYTE_LOG_APPLICATION_VERBOSITY(), "loading settings JSON string '%s'", settings.c_str());
  ...
  if (success)
  {
    ...
    this->Internal->SettingCollectionsAreSorted = false;
    vtkVLogF(VISOCYTE_LOG_APPLICATION_VERBOSITY(), "successfully parsed settings string");
    return true;
  }
  else
  {
...
bool vtkSMSettings::AddCollectionFromFile(const std::string& fileName, double priority)
{
  ...
  if (settingsFile.is_open())
  {
    ...
    vtkVLogF(VISOCYTE_LOG_APPLICATION_VERBOSITY(), "load settings file `%s` -- %s",
      fileName.c_str(), (success ? "succeeded" : "failed"));
    return success;
  }
  else
  {
    vtkVLogF(VISOCYTE_LOG_APPLICATION_VERBOSITY(), "load settings file `%s` -- missing (skipped)",
      fileName.c_str());
    return false;
  }
}
...
bool vtkSMSettings::SaveSettingsToFile(const std::string& filePath)
{
  if (this->Internal->SettingCollections.size() == 0 || !this->Internal->IsModified)
  {
    ...
    vtkVLogIfF(VISOCYTE_LOG_APPLICATION_VERBOSITY(),
  ...
  if (settingsFile.is_open())
  {
    ...
    vtkVLogF(VISOCYTE_LOG_APPLICATION_VERBOSITY(), "saving settings to '%s'", filePath.c_str());
    return true;
  }

  vtkVLogF(
    VISOCYTE_LOG_APPLICATION_VERBOSITY(), "could not save settings to '%s'!", filePath.c_str());
  return false;
}
...
//
// See: https://gitlab.kitware.com/visocyte/visocyte/issues/16938
