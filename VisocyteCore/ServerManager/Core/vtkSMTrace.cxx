  ...
  , Internals(new vtkSMTrace::vtkInternals())
{
  ...
  this->Internals->TraceModule.TakeReference(PyImport_ImportModule("visocyte.smtrace"));
  if (!this->Internals->TraceModule)
  {
    vtkErrorMacro("Failed to import visocyte.smtrace module.");
  }
  else
  {
    ...
    {
      vtkErrorMacro(
        "Failed to locate the _create_trace_item_internal function in visocyte.smtrace module.");
    ...
    {
      vtkErrorMacro("Failed to locate the Untraceable exception class in visocyte.smtrace module.");
      this->Internals->TraceModule.TakeReference(NULL);
...
vtkStdString vtkSMTrace::GetState(int propertiesToTraceOnCreate, bool skipHiddenRepresentations)
{
  ...
  try
  {
    vtkSmartPyObject module(PyImport_ImportModule("visocyte.smstate"));
    if (!module || PyErr_Occurred())
    {
      vtkGenericWarningMacro("Failed to import visocyte.smstate module.");
      throw 1;
    }
