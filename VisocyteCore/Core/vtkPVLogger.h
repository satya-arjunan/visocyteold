...
/**
 * @class vtkPVLogger
 * @brief defines various logging categories for Visocyte
 *
 * Visocyte code generates informative log entires under several categories.
 * This class provides ability to convert a category to a verbosity level
 * to use to log a message.
 *
 * All code in Visocyte that generates informative log messages should use one
 ...
 * `VISOCYTE_LOG_RENDERING_VERBOSITY()` as follows:
 *
 * @code{cpp}
 * vtkVLogF(VISOCYTE_LOG_RENDERING_VERBOSITY(), "message about rendering");
 * @endcode
 *
 ...
 * default, set the environment variable `VISOCYTE_LOG_RENDERING_VERBOSITY` to
 * `INFO` or `0`.
 *
...
public:
   ...
   * `VISOCYTE_LOG_PIPELINE_VERBOSITY` to the expected verbosity level.
   */
  ...
  //@{
  /**
   * Use this verbosity level to for log message relating to Visocyte's plugin
   * system.
   *
   * Default level is `vtkLogger::VERBOSITY_TRACE` unless overridden by calling
   * `SetPluginVerbosity` or by setting the environment variable
   * `VISOCYTE_LOG_PLUGIN_VERBOSITY` to the expected verbosity level.
   */
   ...
   * `VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY` to the expected verbosity level.
   */
   ...
   * `VISOCYTE_LOG_RENDERING_VERBOSITY` to the expected verbosity level.
   */
   ...
   * Default level is `vtkLogger::VERBOSITY_TRACE` unless overridden by calling
   * `SetRenderingVerbosity` or by setting the environment variable
   * `VISOCYTE_LOG_APPLICATION_VERBOSITY` to the expected verbosity level.
   */
  ...
  //@{
  /**
   * Change default verbosity to use for all Visocyte categories defined here if
   * no overrides are specified. This is intended to be used by Visocyte-based
   * applications to change the level at which Visocyte messages are logged in
   * bulk.
   *
 ...
 *
 * @code{cpp}
 *  vtkVLogF(VISOCYTE_LOG_PIPELINE_VERBOSITY(), "pipeline updated");
 * @endcode
 */
#define VISOCYTE_LOG_PIPELINE_VERBOSITY() vtkPVLogger::GetPipelineVerbosity()
 ...
 *
 * @code{cpp}
 *  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "plugin loaded `%s`", name);
 * @endcode
 */
#define VISOCYTE_LOG_PLUGIN_VERBOSITY() vtkPVLogger::GetPluginVerbosity()
 ...
 *
 * @code{cpp}
 *  vtkVLogF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "moving data");
 * @endcode
 */
#define VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY() vtkPVLogger::GetDataMovementVerbosity()
 ...
 *
 * @code{cpp}
 *  vtkVLogF(VISOCYTE_LOG_RENDERING_VERBOSITY(), "still render");
 * @endcode
 */
#define VISOCYTE_LOG_RENDERING_VERBOSITY() vtkPVLogger::GetRenderingVerbosity()
 ...
 *
 * @code{cpp}
 *  vtkVLogF(VISOCYTE_LOG_APPLICATION_VERBOSITY(), "reading config file %s", filename);
 * @endcode
 */
#define VISOCYTE_LOG_APPLICATION_VERBOSITY() vtkPVLogger::GetApplicationVerbosity()
