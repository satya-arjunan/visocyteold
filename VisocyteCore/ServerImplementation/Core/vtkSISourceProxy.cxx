...
void vtkSISourceProxy::UpdatePipeline(int port, double time, bool doTime)
{
  ...
  vtkVLogScopeF(VISOCYTE_LOG_PIPELINE_VERBOSITY(), "%s: update pipeline(%d, %f, %s) ",
...
void vtkSISourceProxy::UpdatePipelineInformation()
{
  ...
  vtkVLogScopeF(VISOCYTE_LOG_PIPELINE_VERBOSITY(), "%s: update pipeline information",
...
void vtkSISourceProxy::MarkStartEvent()
{
  ...
  vtkVLogStartScopeF(VISOCYTE_LOG_PIPELINE_VERBOSITY(), vtkLogIdentifier(this), "%s: execute",
    this->GetLogNameOrDefault());
}
