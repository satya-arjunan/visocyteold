  ...
  : Internals(new vtkPVPythonAlgorithmPlugin::vtkInternals())
{
  ...
  vtkSmartPyObject pvdetail(PyImport_ImportModule("visocyte.detail.pythonalgorithm"));
  if (!pvdetail)
  {
    throw std::runtime_error("Failed to import `visocyte.detail.pythonalgorithm`.");
  }

  vtkSmartPyObject load_plugin(PyObject_GetAttrString(pvdetail, "load_plugin"));
  if (!load_plugin)
  {
    throw std::runtime_error("Failed to locate `visocyte.detail.pythonalgorithm.load_plugin`.");
  }
  ...
  if (!module)
  {
    throw std::runtime_error("Failed to call `visocyte.detail.pythonalgorithm.load_plugin`.");
  }

  vtkSmartPyObject get_plugin_xmls(PyObject_GetAttrString(pvdetail, "get_plugin_xmls"));
  if (!get_plugin_xmls)
  {
    throw std::runtime_error("Failed to locate `visocyte.detail.pythonalgorithm.get_plugin_xmls`.");
  }
  ...
  if (!result)
  {
    throw std::runtime_error("Failed to call `visocyte.detail.pythonalgorithm.get_plugin_xmls`.");
  }
  ...
  if (!get_plugin_name)
  {
    throw std::runtime_error("Failed to locate `visocyte.detail.pythonalgorithm.get_plugin_name`.");
  }
  ...
  {
    throw std::runtime_error("Failed to call `visocyte.detail.pythonalgorithm.get_plugin_name`.");
  }
  ...
  if (!get_plugin_version)
  {
    throw std::runtime_error(
      "Failed to locate `visocyte.detail.pythonalgorithm.get_plugin_version`.");
  }
  ...
  {
    throw std::runtime_error(
      "Failed to call `visocyte.detail.pythonalgorithm.get_plugin_version`.");
  }
