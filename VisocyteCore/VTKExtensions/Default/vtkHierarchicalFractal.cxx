...
//----------------------------------------------------------------------------
int vtkHierarchicalFractal::RequestDataObject(
...
void vtkHierarchicalFractal::SetBlockInfo(vtkUniformGrid* grid, int level, int* ext, int onFace[6])
{
  // onFace[0]:xmin
  // onFace[1]:xmax
  // onFace[2]:ymin
...
//----------------------------------------------------------------------------
// This handles any alterations necessary for ghost levels.
void vtkHierarchicalFractal::SetRBlockInfo(
  vtkRectilinearGrid* grid, int level, int* ext, int onFace[6])
{
  if (this->GhostLevels)
  {
    if (!onFace[0])
...
// This is called by the superclass.
// This is the method you should override.
int vtkHierarchicalFractal::RequestData(vtkInformation* vtkNotUsed(request),
...
//----------------------------------------------------------------------------
int vtkHierarchicalFractal::LineTest2(
...
//----------------------------------------------------------------------------
int vtkHierarchicalFractal::LineTest(
...
//----------------------------------------------------------------------------
void vtkHierarchicalFractal::Traverse(int& blockId, int level, vtkCompositeDataSet* output, int x0,
  int x3, int y0, int y3, int z0, int z3, int onFace[6])
{
  double bds[6];
  ...
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
  {
    if (!this->GenerateRectilinearGrids)
    {
      ...
      fractalSource->SetOriginCX(origin[0] + (spacing[0] * 0.5), origin[1] + (spacing[1] * 0.5),
        origin[2] + (spacing[2] * 0.5), this->TimeStep / 10.0);
...
void vtkHierarchicalFractal::AddBlockIdArray(vtkCompositeDataSet* output)
{
  ...
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem(), blockId++)
  {
    vtkUniformGrid* grid;
    grid = vtkUniformGrid::SafeDownCast(iter->GetCurrentDataObject());
    if (!grid)
...
void vtkHierarchicalFractal::AddGhostLevelArray(vtkDataSet* grid, int dim[3], int onFace[6])
{
...
//----------------------------------------------------------------------------
void vtkHierarchicalFractal::CellExtentToBounds(int level, int ext[6], double bds[6])
{
  int spacingFactor = 1;
  double spacing[3];
  spacingFactor = spacingFactor << level;
...
//----------------------------------------------------------------------------
void vtkHierarchicalFractal::ExecuteRectilinearMandelbrot(vtkRectilinearGrid* grid, double* ptr)
{
  int a0 = 0;
  int a1 = 1;
  int a2 = 2;
...
//----------------------------------------------------------------------------
void vtkHierarchicalFractal::GetContinuousIncrements(
