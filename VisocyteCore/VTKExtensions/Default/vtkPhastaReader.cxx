...
void vtkPhastaReader::SetFieldInfo(const char* visocyteFieldTag, const char* phastaFieldTag,
  int index, int numOfComps, int dataDependency, const char* dataType)
{
  vtkPhastaReaderInternal::FieldInfo& info = this->Internal->FieldInfoMap[visocyteFieldTag];
  ...
  char* fieldFileName, int, vtkUnstructuredGrid* output, int& noOfDatas)
{
  ...
  for (; it != itend; it++)
  {
    const char* visocyteFieldTag = it->first.c_str();
    ...
    dataArray->SetName(visocyteFieldTag);
    ...
    if (index < 0 || index > numOfVars - 1)
    {
      vtkErrorMacro("index [" << index << "] is out of range [num. of vars.:" << numOfVars
                              << "] for field [visocyte field tag:" << visocyteFieldTag
    ...
    { // data is type double
      ...
      switch (numOfComps)
      {
        case 1:
        {
          ...
          if (!activeScalars)
            field->SetActiveScalars(visocyteFieldTag);
          else
            activeScalars = 1;
        ...
        case 3:
        {
          ...
          if (!activeScalars)
            field->SetActiveVectors(visocyteFieldTag);
          else
            activeScalars = 1;
          for (i = 0; i < noOfDatas; i++)
          {
        ...
        case 9:
        {
          ...
          if (!activeTensors)
            field->SetActiveTensors(visocyteFieldTag);
          else
            activeTensors = 1;
          for (i = 0; i < noOfDatas; i++)
          {
