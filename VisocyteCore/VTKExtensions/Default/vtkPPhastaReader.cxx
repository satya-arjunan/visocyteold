  ...
  for (unsigned int i = 0; i < numElements; i++)
  {
    ...
    if (strcmp("Fields", nested->GetName()) == 0)
    {
      ...
      for (unsigned int j = 0; j < numElements2; j++)
      {
        ...
        {
          numberOfFields2++;
          std::string visocyteFieldTagStr, dataTypeStr;
          const char* visocyteFieldTag = 0;
          visocyteFieldTag = nested2->GetAttribute("visocyte_field_tag");
          if (!visocyteFieldTag)
          {
            std::ostringstream visocyteFieldTagStrStream;
            visocyteFieldTagStrStream << "Field " << numberOfFields2 << ends;
            visocyteFieldTagStr = visocyteFieldTagStrStream.str();
            visocyteFieldTag = visocyteFieldTagStr.c_str();
          }
          ...
          this->Reader->SetFieldInfo(
            visocyteFieldTag, phastaFieldTag, index, numOfComps, dataDependency, dataType);
        }
      }
