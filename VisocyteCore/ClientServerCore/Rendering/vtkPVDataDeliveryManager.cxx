  ...
  public:
    ...
    bool Redistribute(int data_distribution_mode, vtkPKdTree* tree, const std::string debugName)
    {
        ...
        this->RedistributedDataObject->GetMTime() < deliveredDataObject->GetMTime())
      {
        ...
        vtkVLogF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "redistribute: %s", debugName.c_str());
...
{
  vtkVLogScopeF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "check for delivery (interactive=%s)",
  ...
  for (iter = this->Internals->ItemsMap.begin(); iter != this->Internals->ItemsMap.end(); ++iter)
  {
    if (this->Internals->IsRepresentationVisible(iter->first.first))
    {
      ...
      {
        vtkVLogF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "needs-delivery: %s",
  ...
  vtkVLogIfF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), keys_to_deliver.size() == 0, "none");
...

{
  ...
  vtkVLogScopeF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "%s data migration",
  ...
  for (unsigned int cc = 0; cc < size; cc += 2)
  {
    ...
    vtkVLogScopeF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "move-data: %s",
...
void vtkPVDataDeliveryManager::RedistributeDataForOrderedCompositing(bool use_lod)
{
  ...
  if (this->RenderView->GetUpdateTimeStamp() > this->RedistributionTimeStamp)
  {
    ...
    {
      vtkVLogScopeF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "regenerate kd-tree");
    ...
    {
      vtkVLogF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(),
  ...
  if (!anything_moved)
  {
    vtkVLogF(VISOCYTE_LOG_DATA_MOVEMENT_VERBOSITY(), "no redistribution was done.");
  }
}
