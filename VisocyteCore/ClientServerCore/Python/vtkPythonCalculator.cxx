...
void vtkPythonCalculator::Exec(const char* expression)
{
  ...
  vtkSmartPyObject modCalculator(PyImport_ImportModule("visocyte.detail.calculator"));
  CheckAndFlushPythonErrors();
  if (!modCalculator)
  {
    vtkErrorMacro("Failed to import `visocyte.detail.calculator` module.");
    return;
  }
  ...
  // call `visocyte.detail.calculator.execute(self)`
