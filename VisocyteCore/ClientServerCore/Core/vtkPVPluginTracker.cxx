...
{
  vtkVLogScopeF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "looking for plugin '%s'", plugin);
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "VTK libraries location is '%s'", vtklib.c_str());
  ...
  locator->SetLogVerbosity(VISOCYTE_LOG_PLUGIN_VERBOSITY());

  // First try the test plugin path, if it exists.
  vtkPVOptions* options = pm->GetOptions();
  if (options->GetTestPluginPath() && strlen(options->GetTestPluginPath()) > 0)
  {
    vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "check `test-plugin-path` first.");
  ...
  if (!vtklib.empty())
  {
    vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "check various prefixes relative to VTK libraries");
    auto pluginpath =
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "failed!!!");
  return std::string();
}
...
vtkPVPluginTracker::vtkPVPluginTracker()
{
  ...
  if (vtksys::SystemTools::GetEnv("PV_PLUGIN_DEBUG") != nullptr)
  {
    vtkWarningMacro("`PV_PLUGIN_DEBUG` environment variable has been deprecated. "
                    "Please use `VISOCYTE_LOG_PIPELINE_VERBOSITY=INFO` instead.");
...
vtkPVPluginTracker* vtkPVPluginTracker::GetInstance()
{
  ...
  if (Instance.GetPointer() == NULL)
  {
    ...
    {
      vtkVLogF(
        VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Could not find `.plugins` file for distributed plugins.");
    }
  }
  return Instance;
}
...
void vtkPVPluginTracker::LoadPluginConfigurationXML(const char* filename, bool forceLoad)
{
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Loading plugin configuration xml `%s`.", filename);
  ...
  const char* xmlcontents, bool forceLoad)
{
  ...
  {
    vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "string not a valid xml -- failed!");
    return;
  }
...
void vtkPVPluginTracker::LoadPluginConfigurationXML(vtkPVXMLElement* root, bool forceLoad)
{
  ...
  for (unsigned int cc = 0; cc < root->GetNumberOfNestedElements(); cc++)
  {
    ...
    if (child && child->GetName() && strcmp(child->GetName(), "Plugin") == 0)
    {
      ...
      {
        vtkVLogF(
          VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Missing required attribute name. Skipping element.");
        continue;
      }
      vtkVLogF(
        VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Trying to locate plugin with name `%s`", name.c_str());
      ...
      if (plugin_filename.empty())
      {
        ...
        vtkVLogF(
          VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Failed to locate file plugin `%s`", name.c_str());
        continue;
      }
      vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "found `%s`", plugin_filename.c_str());
