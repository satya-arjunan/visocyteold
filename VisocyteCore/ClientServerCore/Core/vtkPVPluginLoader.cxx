...
vtkPVPluginLoader::vtkPVPluginLoader()
{
  ...
  {
    paths += env;
    vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "PV_PLUGIN_PATH: %s", env);
  }

#ifdef VISOCYTE_PLUGIN_LOADER_PATHS
  if (!paths.empty())
  {
    paths += ENV_PATH_SEP;
  }
  paths += VISOCYTE_PLUGIN_LOADER_PATHS;
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "VISOCYTE_PLUGIN_LOADER_PATHS: %s",
    VISOCYTE_PLUGIN_LOADER_PATHS);
#endif
  ...
  if (opt)
  {
    ...
    if (appDir.size())
    {
      ...
      paths += appDir;
      vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "appDir: %s", appDir.c_str());
    }
...
void vtkPVPluginLoader::LoadPluginsFromPluginSearchPath()
{
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Loading Plugins from standard PLUGIN_PATHS\n%s",
...
#else
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Static build. Skipping PLUGIN_PATHS.");
#endif
}
...
#else
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Static build. Skipping PV_PLUGIN_CONFIG_FILE.");
#endif
}
//-----------------------------------------------------------------------------
void vtkPVPluginLoader::LoadPluginsFromPath(const char* path)
{
  vtkVLogIfF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), path != nullptr, "Loading plugins in Path: %s", path);

  vtkNew<vtkPDirectory> dir;
  if (dir->Load(path) == false)
  {
    vtkVLogIfF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), path != nullptr, "Invalid directory: %s", path);
    return;
  }
...
bool vtkPVPluginLoader::LoadPluginInternal(const char* file, bool no_errors)
{
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Attempting to load: %s", file);
  ...
  {
    vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Loading XML plugin.");
  ...
  if (!lib)
  {
    ...
    vtkVLogIfF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), this->ErrorString != nullptr,
  ...
  vtkVLogF(VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Plugin's signature: %s", pv_verfication_data.c_str());
  ...
  if (!pluginPathPresent)
  {
    ...
    vtkVLogF(
      VISOCYTE_LOG_PLUGIN_VERBOSITY(), "Updating Shared Library Paths: %s", ldLibPath.c_str());
  }
