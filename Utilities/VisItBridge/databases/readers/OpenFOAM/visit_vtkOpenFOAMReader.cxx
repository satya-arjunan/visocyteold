/*=========================================================================
  ...
  Module:    visit_vtkOpenFOAMReader.cxx
...
// the reader core of visit_vtkOpenFOAMReader
class visit_vtkOpenFOAMReaderPrivate : public vtkObject
{
public:
  static visit_vtkOpenFOAMReaderPrivate *New();
  vtkTypeMacro(visit_vtkOpenFOAMReaderPrivate, vtkObject);
  ...
  bool MakeInformationVector(const vtkStdString &, const vtkStdString &,
      const vtkStdString &, visit_vtkOpenFOAMReader *);
  ...
  void SetupInformation(const vtkStdString &, const vtkStdString &,
      const vtkStdString &, visit_vtkOpenFOAMReaderPrivate *);
...
private:
  ...
  visit_vtkOpenFOAMReaderPrivate();
  ~visit_vtkOpenFOAMReaderPrivate();

  // not implemented.
  visit_vtkOpenFOAMReaderPrivate(const visit_vtkOpenFOAMReaderPrivate &);
  void operator=(const visit_vtkOpenFOAMReaderPrivate &);
...
// Visocyte3/VTK/Utilities/vtksqlite/vtk_sqlite3.c
...
void visit_vtkOpenFOAMReaderPrivate::ClearInternalMeshes()
{
...
void visit_vtkOpenFOAMReaderPrivate::ClearBoundaryMeshes()
{
...
void visit_vtkOpenFOAMReaderPrivate::ClearMeshes()
{
  this->ClearInternalMeshes();
  this->ClearBoundaryMeshes();
}

void visit_vtkOpenFOAMReaderPrivate::SetTimeValue(const double requestedTime)
{
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::SetupInformation(const vtkStdString &casePath,
    const vtkStdString &regionName, const vtkStdString &procName,
    visit_vtkOpenFOAMReaderPrivate *master)
{
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::GetFieldNames(const vtkStdString &tempPath,
...
void visit_vtkOpenFOAMReaderPrivate::LocateLagrangianClouds(
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::SortFieldFiles(vtkStringArray *selections,
...
int visit_vtkOpenFOAMReaderPrivate::MakeMetaDataAtTimeStep(
  ...
  if (this->PolyMeshFacesDir->GetValue(this->TimeStep)
    ...
    if (boundaryDict == NULL)
      ...
      for (size_t i = 0; i < boundaryDict->size(); i++)
        ...
        if (nFacesEntry == NULL)
          {
          vtkDebugMacro(<< "nFaces entry not found in boundary entry "
        ...
        if (startFaceEntry == NULL)
          {
          vtkDebugMacro(<< "startFace entry not found in boundary entry "
              << boundaryEntryI->GetKeyword().c_str());
          delete boundaryDict;
          return 0;
          }
        ...
        if (typeEntry == NULL)
          {
          vtkDebugMacro(<< "type entry not found in boundary entry "
              << boundaryEntryI->GetKeyword().c_str());
          delete boundaryDict;
          return 0;
          }
...
bool visit_vtkOpenFOAMReaderPrivate::ListTimeDirectoriesByControlDict(
    vtkFoamDict* dictPtr)
{
  ...
  if (startTimeEntry == NULL)
    {
    vtkDebugMacro(<< "startTime entry not found in controlDict");
    return false;
    }
  ...
  if (endTimeEntry == NULL)
    {
    vtkDebugMacro(<< "endTime entry not found in controlDict");
    return false;
    }
  ...
  if (deltaTEntry == NULL)
    {
    vtkDebugMacro(<< "deltaT entry not found in controlDict");
    return false;
    }
  ...
  if (writeIntervalEntry == NULL)
    {
    vtkDebugMacro(<< "writeInterval entry not found in controlDict");
    return false;
    }
  ...
  if (timeFormatEntry == NULL)
    {
    vtkDebugMacro(<< "timeFormat entry not found in controlDict");
    return false;
    }
  ...
  if (writeControlEntry == NULL)
    {
    vtkDebugMacro(<< "writeControl entry not found in controlDict");
    return false;
    }
  ...
  if (writeControl == "timeStep")
    ...
    {
    vtkDebugMacro(<<"Time step can't be determined because writeControl is"
...
// case directory
bool visit_vtkOpenFOAMReaderPrivate::ListTimeDirectoriesByInstances()
{
  ...
  if (!test->Open(this->CasePath.c_str()))
    {
    test->Delete();
    vtkDebugMacro(<< "Can't open directory " << this->CasePath.c_str());
    return false;
    }
...
bool visit_vtkOpenFOAMReaderPrivate::MakeInformationVector(
    const vtkStdString &casePath, const vtkStdString &controlDictPath,
    const vtkStdString &procName, visit_vtkOpenFOAMReader *parent)
{
  this->CasePath = casePath;
  this->ProcessorName = procName;
  this->Parent = parent;
  ...
  if (this->Parent->GetListTimeStepsByControlDict())
    ...
    if (!io.Open(controlDictPath))
      {
      vtkDebugMacro(<<"Error opening " << io.GetFileName().c_str() << ": "
          << io.GetError().c_str());
      return false;
      }
    vtkFoamDict dict;
    ...
    if (dict.GetType() != vtkFoamToken::DICTIONARY)
      {
      vtkDebugMacro(<<"The file type of " << io.GetFileName().c_str()
          << " is not a dictionary");
      return false;
      }
    ...
    if (writeControlEntry == NULL)
      {
      vtkDebugMacro(<< "writeControl entry not found in "
  ...
  // upon refresh since Visocyte rewinds TimeStep to 0, but for precaution
  if (this->TimeValues->GetNumberOfTuples() > 0)
    {
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::AppendMeshDirToArray(
...
void visit_vtkOpenFOAMReaderPrivate::PopulatePolyMeshDirArrays()
{
...
vtkFloatArray* visit_vtkOpenFOAMReaderPrivate::ReadPointsFile()
{
  // path to points file
...
vtkFoamIntVectorVector * visit_vtkOpenFOAMReaderPrivate::ReadFacesFile(
...
vtkFoamIntVectorVector * visit_vtkOpenFOAMReaderPrivate::ReadOwnerNeighborFiles(
    const vtkStdString &ownerNeighborPath, vtkFoamIntVectorVector *facePoints)
{
  ...
  if (io.Open(ownerPath) || io.Open(ownerPath + ".gz"))
    ...
    if (nFaces < nNeiFaces)
      {
      vtkDebugMacro(<<"Numbers of owner faces " << nFaces
    ...
    for (int cellI = 0; cellI < this->NumCells; cellI++)
      ...
      for (int faceI = 0; faceI < nCellFaces; faceI++)
        ...
        if (f < 0 || f >= nFaces) // make sure the face number is valid
          {
          vtkDebugMacro("Face number " << f << " in cell " << cellI
    ...
    for (int faceI = 0; faceI < nFaces; faceI++)
      ...
      if (this->FaceOwner->GetValue(faceI) == -1)
        {
        vtkDebugMacro(<<"Face " << faceI << " is not used");
        this->FaceOwner->Delete();
        this->FaceOwner = NULL;
        delete cells;
        return NULL;
...
//-----------------------------------------------------------------------------
bool visit_vtkOpenFOAMReaderPrivate::CheckFacePoints(
    vtkFoamIntVectorVector *facePoints)
{
  ...
  for (int faceI = 0; faceI < nFaces; faceI++)
    ...
    if (nPoints < 3)
      {
      vtkDebugMacro(<< "Face " << faceI << " has only " << nPoints
    ...
    for (int pointI = 0; pointI < nPoints; pointI++)
      ...
      if (p < 0 || p >= this->NumPoints)
        {
        vtkDebugMacro(<< "The point number " << p << " at face number " << faceI
...
void visit_vtkOpenFOAMReaderPrivate::InsertCellsToGrid(
    vtkUnstructuredGrid* internalMesh,
    ...
    // OFpolyhedron || vtkConvexPointSet
      ...
      if (additionalCells != NULL) // decompose into tets and pyramids
        ...
        if (nPoints > maxNPoints || nPolyPoints > maxNPolyPoints)
          {
          vtkDebugMacro(<< "Too large polyhedron at cellId = " << cellId);
          cellPoints->Delete();
          polyPoints->Delete();
          return;
          }
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::SetBlockName(vtkMultiBlockDataSet *blocks,
...
vtkUnstructuredGrid *visit_vtkOpenFOAMReaderPrivate::MakeInternalMesh(
    const vtkFoamIntVectorVector *cellsFaces,
    const vtkFoamIntVectorVector *facesPoints, vtkFloatArray *pointArray)
{
  // Create Mesh
...
void visit_vtkOpenFOAMReaderPrivate::InsertFacesToGrid(vtkPolyData *boundaryMesh,
...
vtkMultiBlockDataSet *visit_vtkOpenFOAMReaderPrivate::MakeBoundaryMesh(
    const vtkFoamIntVectorVector *facesPoints, vtkFloatArray* pointArray)
{
  ...
  for (int boundaryI = 0; boundaryI < nBoundaries; boundaryI++)
    ...
    if (startFace < 0 || nFaces < 0)
      {
      vtkDebugMacro(<<"Neither of startFace " << startFace << " nor nFaces "
          << nFaces << " can be negative for patch " << beI.BoundaryName.c_str());
      return NULL;
      }
    if (previousEndFace >= 0 && previousEndFace != startFace)
      {
      vtkDebugMacro(<<"The end face number " << previousEndFace - 1
          << " of patch "
...
void visit_vtkOpenFOAMReaderPrivate::TruncateFaceOwner()
{
  const int boundaryStartFace =
...
template <typename T1, typename T2> bool visit_vtkOpenFOAMReaderPrivate::ExtendArray(
...
vtkPoints *visit_vtkOpenFOAMReaderPrivate::MoveInternalMesh(
    vtkUnstructuredGrid *internalMesh, vtkFloatArray *pointArray)
{
  ...
  if (internalMesh->GetPoints()->GetNumberOfPoints() != pointArray->GetNumberOfTuples())
    {
    vtkDebugMacro(<< "The numbers of points for old points "
...
void visit_vtkOpenFOAMReaderPrivate::MoveBoundaryMesh(
...
void visit_vtkOpenFOAMReaderPrivate::InterpolateCellToPoint(vtkFloatArray *pData,
...
//-----------------------------------------------------------------------------
bool visit_vtkOpenFOAMReaderPrivate::ReadFieldFile(vtkFoamIOobject *ioPtr,
    ...
    vtkDataArraySelection *selection)
{
  ...
  if (dict.GetType() != vtkFoamToken::DICTIONARY)
    {
    vtkDebugMacro(<<"File " << io.GetFileName().c_str()
...
//-----------------------------------------------------------------------------
vtkFloatArray *visit_vtkOpenFOAMReaderPrivate::FillField(vtkFoamEntry *entryPtr,
  ...
  if (entry.FirstValue().GetIsUniform() || entry.GetKeyword() == "uniformValue")
    ...
    if (entry.FirstValue().GetType() == vtkFoamToken::SCALAR || entry.FirstValue().GetType() == vtkFoamToken::LABEL)
      ...
      if (entry.FirstValue().GetType() == vtkFoamToken::LABELLIST)
        ...
        {
        vtkDebugMacro(<<"Wrong list type for uniform field");
        return NULL;
        }
    ...
    if ((fieldType == "ScalarField" && entry.FirstValue().GetType() == vtkFoamToken::SCALARLIST) || ((fieldType
      ...
      if (nTuples != nElements)
        {
        vtkDebugMacro(<<"Number of cells/points in mesh and field don't match: "
      ...
      {
      vtkDebugMacro(<< ioPtr->GetFileName().c_str() << " is not a valid "
...
void visit_vtkOpenFOAMReaderPrivate::ConstructDimensions(vtkStdString *dimString,
    vtkFoamDict *dictPtr)
{
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::GetVolFieldAtTimeStep(
    ...
    const vtkStdString &varName)
{
  ...
  if (io.GetClassName().substr(0, 3) != "vol")
    {
    vtkDebugMacro(<< io.GetFileName().c_str() << " is not a volField");
    return;
    }
  ...
  if (iEntry == NULL)
    {
    vtkDebugMacro(<<"internalField not found in " << io.GetFileName().c_str());
    return;
    }
  ...
  if (iEntry->FirstValue().GetType() == vtkFoamToken::EMPTYLIST)
    ...
    if (this->NumCells > 0)
      {
      vtkDebugMacro(<<"internalField of " << io.GetFileName().c_str()
          << " is empty");
      }
    return;
    }
  ...
  if (bEntry == NULL)
    {
    vtkDebugMacro(<< "boundaryField not found in object " << varName.c_str()
  ...
  for (int boundaryI = 0, activeBoundaryI = 0; boundaryI
    ...
    if (bEntryI == NULL)
      {
      vtkDebugMacro(<< "boundaryField " << boundaryNameI.c_str()
    ...
    if (bEntryI->FirstValue().GetType() != vtkFoamToken::DICTIONARY)
      {
      vtkDebugMacro(<< "Type of boundaryField " << boundaryNameI.c_str()
...
void visit_vtkOpenFOAMReaderPrivate::GetPointFieldAtTimeStep(
    vtkUnstructuredGrid *internalMesh, vtkMultiBlockDataSet *boundaryMesh,
    const vtkStdString &varName)
{
  vtkFoamIOobject io(this->CasePath);
  ...
  if (io.GetClassName().substr(0, 5) != "point")
    {
    vtkDebugMacro(<< io.GetFileName().c_str() << " is not a pointField");
    return;
    }
...
//-----------------------------------------------------------------------------
vtkMultiBlockDataSet* visit_vtkOpenFOAMReaderPrivate::MakeLagrangianMesh()
{
  ...
  for (int cloudI = 0; cloudI
    ...
    for (int fieldI = 0; fieldI
      ...
      if (!dict2.ReadField(io2))
        {
        vtkDebugMacro(<<"Error reading line " << io2.GetLineNumber()
      ...
      if (dict2.GetType() != vtkFoamToken::SCALARLIST && dict2.GetType()
        ...
        {
        vtkDebugMacro(<< io2.GetFileName().c_str()
...
vtkFoamDict* visit_vtkOpenFOAMReaderPrivate::GatherBlocks(const char* typeIn, bool mustRead)
{
...
bool visit_vtkOpenFOAMReaderPrivate::GetPointZoneMesh(
    vtkMultiBlockDataSet *pointZoneMesh, vtkPoints *points)
{
  ...
  for (int i = 0; i < nPointZones; i++)
    ...
    if (pointLabelsEntry == NULL)
      ...
      vtkDebugMacro(<<"pointLabels not found in pointZones");
      return false;
      }
    ...
    if (pointLabelsEntry->FirstValue().GetType() != vtkFoamToken::LABELLIST)
      {
      delete pointZoneDictPtr;
      vtkDebugMacro(<<"pointLabels not of type labelList: type = "
    ...
    if (nPoints > this->NumPoints)
      {
      vtkDebugMacro(<<"The length of pointLabels " << nPoints
...
bool visit_vtkOpenFOAMReaderPrivate::GetFaceZoneMesh(
    vtkMultiBlockDataSet *faceZoneMesh,
    const vtkFoamIntVectorVector *facesPoints, vtkPoints *points)
{
  ...
  for (int i = 0; i < nFaceZones; i++)
    ...
    if (faceLabelsEntry == NULL)
      ...
      vtkDebugMacro(<<"faceLabels not found in faceZones");
      return false;
      }
    ...
    if (faceLabelsEntry->FirstValue().GetType() != vtkFoamToken::LABELLIST)
      {
      delete faceZoneDictPtr;
      vtkDebugMacro(<<"faceLabels not of type labelList");
      return false;
      }
    ...
    if (nFaces > this->FaceOwner->GetNumberOfTuples())
      {
      vtkDebugMacro(<<"The length of faceLabels " << nFaces
...
bool visit_vtkOpenFOAMReaderPrivate::GetCellZoneMesh(
    vtkMultiBlockDataSet *cellZoneMesh,
    const vtkFoamIntVectorVector *cellsFaces,
    const vtkFoamIntVectorVector *facesPoints, vtkPoints *points)
{
  ...
  for (int i = 0; i < nCellZones; i++)
    ...
    if (cellLabelsEntry == NULL)
      ...
      vtkDebugMacro(<<"cellLabels not found in cellZones");
      return false;
      }
    ...
    if (cellLabelsEntry->FirstValue().GetType() != vtkFoamToken::LABELLIST)
      {
      delete cellZoneDictPtr;
      vtkDebugMacro(<<"cellLabels not of type labelList");
      return false;
      }
    ...
    if (nCells > this->NumCells)
      {
      vtkDebugMacro(<<"The length of cellLabels " << nCells
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReaderPrivate::AddArrayToFieldData(
...
int visit_vtkOpenFOAMReaderPrivate::RequestData(vtkMultiBlockDataSet *output,
    bool recreateInternalMesh, bool recreateBoundaryMesh, bool updateVariables)
{
  ...
  const bool createEulerians = this->Parent->PatchDataArraySelection->ArrayExists("internalMesh") || this->RegionName != "";
...
//-----------------------------------------------------------------------------
// CanReadFile
int visit_vtkOpenFOAMReader::CanReadFile(const char *vtkNotUsed(fileName))
{
...
int visit_vtkOpenFOAMReader::GetNumberOfSelectionArrays(vtkDataArraySelection *s)
{
  return s->GetNumberOfArrays();
}

int visit_vtkOpenFOAMReader::GetSelectionArrayStatus(vtkDataArraySelection *s,
    const char *name)
{
...
const char *visit_vtkOpenFOAMReader::GetSelectionArrayName(vtkDataArraySelection *s,
    int index)
{
...
//-----------------------------------------------------------------------------
// RequestInformation
int visit_vtkOpenFOAMReader::RequestInformation(vtkInformation *vtkNotUsed(request), vtkInformationVector **vtkNotUsed(inputVector), vtkInformationVector *outputVector)
{
  if (!this->FileName || strlen(this->FileName) == 0)
    {
    vtkDebugMacro("FileName has to be specified!");
    return 0;
    }
...
//-----------------------------------------------------------------------------
// RequestData
int visit_vtkOpenFOAMReader::RequestData(
    ...
    vtkInformationVector *outputVector)
{
  ...
  // create dataset
  int ret = 1;
  visit_vtkOpenFOAMReaderPrivate *reader;
  // if the only region is not a subregion, omit being wrapped by a
  // multiblock dataset
  if (this->Readers->GetNumberOfItems() == 1 && (reader = visit_vtkOpenFOAMReaderPrivate::SafeDownCast(
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::SetTimeInformation(vtkInformationVector *outputVector,
    vtkDoubleArray *timeValues)
{
...
//-----------------------------------------------------------------------------
int visit_vtkOpenFOAMReader::MakeInformationVector(
    vtkInformationVector *outputVector, const vtkStdString& procName)
{
  *this->FileNameOld = vtkStdString(this->FileName);
  ...
  visit_vtkOpenFOAMReaderPrivate *masterReader = visit_vtkOpenFOAMReaderPrivate::New();
  ...
  if (masterReader->GetTimeValues()->GetNumberOfTuples() == 0)
    {
    vtkDebugMacro(<< this->FileName << " contains no timestep data.");
    masterReader->Delete();
    return 0;
    }
  ...
  if (!dir->Open(constantPath.c_str()))
    {
    vtkDebugMacro(<< "Can't open " << constantPath.c_str());
    return 0;
    }
  for (int fileI = 0; fileI < dir->GetNumberOfFiles(); fileI++)
    ...
    if (subDir != "." && subDir != ".." && dir->FileIsDirectory(subDir.c_str()))
      ...
      if (vtksys::SystemTools::FileExists(boundaryPath.c_str(), true)
        ...
        {
        visit_vtkOpenFOAMReaderPrivate *subReader = visit_vtkOpenFOAMReaderPrivate::New();
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::CreateCasePath(vtkStdString &casePath,
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::AddSelectionNames(vtkDataArraySelection *selections,
...
//-----------------------------------------------------------------------------
bool visit_vtkOpenFOAMReader::SetTimeValue(const double timeValue)
{
  bool modified = false;
  visit_vtkOpenFOAMReaderPrivate *reader;
  this->Readers->InitTraversal();
...
//-----------------------------------------------------------------------------
vtkDoubleArray *visit_vtkOpenFOAMReader::GetTimeValues()
{
  ...
  visit_vtkOpenFOAMReaderPrivate *reader =
      visit_vtkOpenFOAMReaderPrivate::SafeDownCast(this->Readers->GetItemAsObject(0));
  return reader != NULL ? reader->GetTimeValues() : NULL;
}

//-----------------------------------------------------------------------------
int visit_vtkOpenFOAMReader::MakeMetaDataAtTimeStep(const bool listNextTimeStep)
{
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::CreateCharArrayFromString(vtkCharArray *array,
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::UpdateStatus()
{
...
//-----------------------------------------------------------------------------
void visit_vtkOpenFOAMReader::UpdateProgress(double amount)
{
...
vtkStdString visit_vtkOpenFOAMReader::GetCellArrayClassName(const char *name)
{
  vtkStdString ret;
...
vtkStdString visit_vtkOpenFOAMReader::GetPointArrayClassName(const char *name)
{
  vtkStdString ret;
  if (name == NULL || name[0] == '\0')
    {
...
vtkStdString visit_vtkOpenFOAMReader::GetLagrangianArrayClassName(const char *name)
{
  vtkStdString ret;
  if (name == NULL || name[0] == '\0')
    {
...
int visit_vtkOpenFOAMReader::GetCellArrayExists(const char *name)
{
  return this->CellDataArraySelection->ArrayExists(name);
}

int visit_vtkOpenFOAMReader::GetPointArrayExists(const char *name)
{
  return this->PointDataArraySelection->ArrayExists(name);
}

int visit_vtkOpenFOAMReader::GetLagrangianArrayExists(const char *name)
{
  return this->LagrangianDataArraySelection->ArrayExists(name);
}

vtkStdString visit_vtkOpenFOAMReader::GetCellZoneName(const int index)
{
  vtkStdString ret;
...
vtkStdString visit_vtkOpenFOAMReader::GetFaceZoneName(const int index)
{
  vtkStdString ret;
...
vtkStdString visit_vtkOpenFOAMReader::GetPointZoneName(const int index)
{
  vtkStdString ret;
