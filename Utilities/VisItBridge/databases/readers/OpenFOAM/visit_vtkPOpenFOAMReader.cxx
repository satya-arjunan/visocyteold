/*=========================================================================
  ...
  Module:    visit_vtkPOpenFOAMReader.cxx
...
vtkStandardNewMacro(visit_vtkPOpenFOAMReader);

//-----------------------------------------------------------------------------
visit_vtkPOpenFOAMReader::visit_vtkPOpenFOAMReader()
{
  this->NumProcesses = 1;
  this->ProcessId = 0;
...
//-----------------------------------------------------------------------------
void visit_vtkPOpenFOAMReader::PrintSelf(ostream &os, vtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void visit_vtkPOpenFOAMReader::SetCaseType(const int t)
{
  if (this->CaseType != t)
    {
...
//-----------------------------------------------------------------------------
int visit_vtkPOpenFOAMReader::RequestInformation(vtkInformation *request,
    ...
    *this->Superclass::FileNameOld = vtkStdString(this->FileName);
    ...
    if (this->ProcessId == 0)
      ...
      if (procNames->GetNumberOfTuples() > 0)
        {
        visit_vtkOpenFOAMReader *masterReader = visit_vtkOpenFOAMReader::New();
    ...
    for (int procI = (this->ProcessId ? this->ProcessId : this->NumProcesses); procI
      ...
      {
      visit_vtkOpenFOAMReader *subReader = visit_vtkOpenFOAMReader::New();
...
//-----------------------------------------------------------------------------
int visit_vtkPOpenFOAMReader::RequestData(vtkInformation *request,
    vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
  if (this->CaseType == RECONSTRUCTED_CASE)
    {
  ...
  if (this->Superclass::Readers->GetNumberOfItems() > 0)
    ...
    visit_vtkOpenFOAMReader *reader;
    this->Superclass::CurrentReaderIndex = 0;
    this->Superclass::Readers->InitTraversal();
    while ((reader
        = visit_vtkOpenFOAMReader::SafeDownCast(this->Superclass::Readers->GetNextItemAsObject()))
        != NULL)
      {
    ...
    {
    this->GatherMetaData();
    // page 322 of The Visocyte Guide says the output must be initialized
    output->Initialize();
    }
...
//-----------------------------------------------------------------------------
void visit_vtkPOpenFOAMReader::BroadcastStatus(int &status)
{
}

//-----------------------------------------------------------------------------
void visit_vtkPOpenFOAMReader::GatherMetaData()
{
  if (this->NumProcesses > 1)
    {
...
void visit_vtkPOpenFOAMReader::AllGather(vtkStringArray *s)
{
  vtkIdType length = 0;
...
void visit_vtkPOpenFOAMReader::AllGather(vtkDataArraySelection *s)
{
  vtkIdType length = 0;
