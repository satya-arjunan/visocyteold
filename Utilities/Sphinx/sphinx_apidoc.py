...
def format_simple_proxy_directive(prx, package='visocyte', module='simple'):
...
def create_simple_module_files(package, module, proxy, opts):
    ...
    gen_prx = sorted(visocyte.simple._get_generated_proxies())

    # Create file for simple module
    text = format_heading(1, '%s Module' % module)
    text += "For generated server-side proxies, please refer to "\
            ":doc:`visocyte.servermanager_proxies`\n\n"
    ...
    for prx in gen_prx:
        ...
        text += '\n\nFor the full list of servermanager proxies, please refer to '\
                ':doc:`Available readers, sources, writers, filters and animation cues <visocyte.servermanager_proxies>`'
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    for py_file in py_files:
        ...
        if is_package:
            ...
            if master_package == 'visocyte' and py_path == 'simple':
